...
function waterfall(){
    ...
    //$('#main').width(num*iPinW).css('margin','0 auto');

    var pinHArr=[];//用于存储 每列中的所有块框相加的高度。

    $aPin.each( function( index, value ){
        console.log(value);
        console.log($aPin.eq(index));
...
function checkscrollside(){
    var $aPin = $( "#main>div" );
    var lastPinH = $aPin.last().offset().top + Math.floor($aPin.last().outerHeight()/2);//创建【触发添加块框函数waterfall()】的高度：最后一个块框的距离网页顶部+自身高的一半(实现未滚到底就开始加载)
